...
echo -e "\n[2/5] Attaching Service Plugins (basic-auth, rate-limiting)..."
...
curl -s -X POST "$ADMIN_URL/services/$SERVICE_NAME/plugins" \
  -d "name=acl" \
  -d "config.allow[]=dors_group" > /dev/null || true
...
curl -s -X POST "$ADMIN_URL/consumers" \
  ...
  -d "custom_id=site_dors_va" > /dev/null || true

curl -s -X POST "$ADMIN_URL/consumers/$CONSUMER_NAME/basic-auth" \
  -d "username=$DORS_USER" \
  -d "password=$DORS_PASS" > /dev/null || true
...
echo -e "\n[4/5] Creating 4 DORS Routes & Translator Plugins..."
